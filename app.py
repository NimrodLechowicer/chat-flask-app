...
def index():
    return 'chat app by Nimrod Lechowicer'
...
def read_message():
    ...
    if len(df) > 0:
        db.commit_to_db(f'''
        update messages set is_read = true where messages_id = {df.iloc[0]['messages_id'].replacr("'", "''")}
        ''')
...
def delete_message(message_id):
    """
    this route delete a massage from the database if the endpoint allowed to do so
    """
    error_details = 'massage_id not exists'
    df = db.query(f'''
    select *
    from messages
    ...
    if len(df) > 0:
        if len(df.query(f"sender_id == {user_id} or receiver_id == {user_id}")) > 0:
            db.commit_to_db(f'delete from messages where messages_id = {message_id} ')
            return jsonify({'status': 'ok'})
        else:
            error_details = 'no permissions'
